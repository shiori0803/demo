...
class AuthorControllerTest @Autowired constructor(
    ...
    companion object {
        @JvmStatic
        fun invalidNameRequests(): Stream<Arguments> = Stream.of(
        ...
        @JvmStatic
        fun invalidFutureDates(): Stream<Arguments> = Stream.of(
            Arguments.of("Test Name", LocalDate.now()), Arguments.of("Test Name", LocalDate.now().plusDays(1))
        )
    ...
    fun `getBooksByAuthorId 正常にデータを取得できる場合`() {
        // Given
        val authorId = 1L
        val mockAuthorResponse = AuthorResponse(
            id = authorId, name = "Haruki Murakami", birthDate = LocalDate.of(1949, 1, 12)
        )
        val mockAuthorWithBooksResponse = AuthorWithBooksResponse(
            author = mockAuthorResponse, books = emptyList()
        )
    ...
    fun `registerAuthor 正常にデータを登録できる場合`() {
        ...
        val createdAuthorDto = AuthorDto(id = authorId, name = authorName, birthDate = authorBirthDate)
        ...
        // When & Then
        mockMvc.perform(
            post("/api/authors").contentType(MediaType.APPLICATION_JSON)
    ...
    fun `patchAuthor 正常に更新できる場合`(
        originalAuthorDto: AuthorDto, updatedAuthorDto: AuthorDto, updatesMap: Map<String, Any?>
    ) {
        // Given
        ...
        // When & Then
        mockMvc.perform(
            patch("/api/authors/{id}", authorId).contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request))
    ...
    fun `patchAuthor nameがnullまたは空文字、空白の場合に更新値として渡さない`(
        request: PatchAuthorRequest, expectedUpdates: Map<String, Any?>
    ) {
        // Given
        val authorId = 1L
    ...
    fun `patchAuthor birthDateの値が本日を含む未来日だったらバリデーションエラーになる`() {
        ...
        } throws IllegalArgumentException("invalid.argument.birthdate.future")
        ...
        mockMvc.perform(
            ...
            assertThat(resolvedException.message).isEqualTo("invalid.argument.birthdate.future")
        }
        verify(exactly = 1) { authorService.partialUpdateAuthor(authorId, any()) }
    }

    @Test
    fun `patchAuthor レスポンスボディが空だったら更新するものがないエラーになる`() {
        // Given
        val authorId = 1L
